...
from ce_explanation_factual_shap import plugin as shap_plugin_mod
...
def test_plugin_should_be_runtime_consumable(monkeypatch):
    class _DummyShapPipeline:
        ...
        def explain(self, _x, **_kwargs):
            return {"ok": True}

        def is_shap_enabled(self):
            return True
    ...
    descriptor = registry.find_explanation_descriptor("official.explanation.factual.shap")
