...
| `src/calibrated_explanations/ce_agent_utils.py` | Legacy/backward compatibility only; not recommended for new agent behavior. Do not treat as the agent interface. |

> **CE-First reminder**: `WrapCalibratedExplainer` and its `fit → calibrate → explain/predict`
> lifecycle are the public contract. Agents must use this API directly. `ce_agent_utils` is
> retained only for backward compatibility and examples; it does not replace the public API.
