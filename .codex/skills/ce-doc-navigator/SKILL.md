...
→ Route to `ce-pipeline-builder` with a note that `ce_agent_utils` is retained only
  for backward compatibility and examples; agents must use the public API skeleton directly.
...
→ Primary: `ce-pipeline-builder` (use the explicit `WrapCalibratedExplainer` skeleton; do not use `ce_agent_utils`)
→ Note: `ce_agent_utils` is legacy/backward compatibility only — redirect to the public API
