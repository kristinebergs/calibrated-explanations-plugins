...
| BARS-017 | Factual bar colors | CE `color_role` → red/blue for clf, blue/red for regression | `_CLF_POS_COLOR` / `_REG_POS_COLOR` constants | ⚠️ WATCH — add color primitive parity test |
| BARS-021 | Threshold x-labels | `P(y>thr)` / `P(y<=thr)` with `.2f` | Uses `y_threshold`; label strings match core | ⚠️ WATCH — verify exact caption string |
...
| ID | Primitive | PlotSpec / legacy | Plotly (after this session) | Status |
|---|---|---|---|---|
| BARS-004 | Alternative ranking | CE `rank_features` + `calculate_metrics`; flip when base ≤ 0.5; reverse | `_rank_items` re-implements formula inline; does not call CE functions | ❌ GAP — see action below |
...
| BARS-016 | Alternative bar colors | `_legacy_get_fill_color(predict, 0.99)` per segment | **Now uses `_ce_fill_color`** (imported from `calibrated_explanations.viz.builders` with inline fallback) | ✅ CLOSED (this session) |
...
| BARS-025 | `show_uncertainty` for alternatives | No public toggle; intervals always shown | Accepted no-op; documented | ⚠️ WATCH — remove from public docs |

---

## Remaining open gaps

### ❌ BARS-004: Alternative ranking must call CE core

**Risk:** The inline `_rank_items` formula can drift from CE's `rank_features` + `calculate_metrics` as CE evolves. Classification-flip (when `base_predict < 0.5`) and `rnk_weight` blending are especially fragile.

**Action:**
```python
# In _rank_items (or a replacement _compute_alt_ranking):
# 1. For feature_weight: call local_explanation.rank_features(weights, width=widths, num_to_show=filter_top)
# 2. For ensured/uncertainty: call calculate_metrics(uncertainty, prediction, w=rnk_weight, metric=rnk_metric)
#    then call local_explanation.rank_features(width=rank_arr, num_to_show=filter_top)
# 3. Flip prediction when is_classification and base_predict < 0.5 (before passing to calculate_metrics)
# 4. Reverse the returned indices
```
...
Tests live in `tests/parity/test_plotly_bars_parity.py` and pass 24/25 (1 intentional skip — no interval bars).
